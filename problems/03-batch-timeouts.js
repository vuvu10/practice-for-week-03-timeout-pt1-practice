...
function batchTimeouts(callbacks, delays) {
  ...
  for (let i = 0; i < callbacks.length; i++) {
    const delayInMilliseconds = delays[i] * 1000;
    const timeout = setTimeout(callbacks[i], delayInMilliseconds);
    timeouts.push(timeout);
  }
  return timeouts;
}

const sayHello = () => console.log('hi'); //=> 500
const sayGoodbye = () => console.log('bye'); //=> 200
const shout = () => console.log('WHAT?'); //=> 900
const tasks = [sayHello, sayGoodbye, shout];
const delays = [500, 200, 900];
